...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json()) as { p1?: string; p2?: string; p3?: string; seconds?: number; previous_decode_id?: string };
    const decode = await composeDraft(
      token.circleId,
      token.memberId,
      {
        p1: body.p1 || "",
